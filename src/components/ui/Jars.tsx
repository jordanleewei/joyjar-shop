...
export const JAR_STYLES = ["classic", "jam", "apothecary", "premium", "mason", "minimal"];
const GOLD = "#bfa063", GOLD_LT = "#e7d3a0", GOLD_DK = "#8a6f33";

function jarColors(tone: string) {
  const T: any = TONES || {};
  return T[tone] || T.matcha || { dessert: "#cdd9b4", dessertTop: "#e3ead0", label: "#7d6a3c", labelInk: "#fbf7ee" };
}

// label text on the band
function BandText({ cx, cy, label, labelEn, ink, big = 21, font = "'Noto Serif TC', serif", enColor }: any) {
  return (
    <React.Fragment>
...
function Scales({ x, y, w, color, rows = 1 }: any) {
  const r = 7, gap = r * 2;
...
function GlassSheen({ id, x, y, w, h, rx }: any) {
  return <rect x={x} y={y} width={w} height={h} rx={rx} fill={`url(#${id})`} />;
}

function Defs({ tone, st }: any) {
  const sid = `${st}-${tone}`;
  return (
    <defs>
...
function renderJar(st: string, tone: string, label: string, labelEn: string) {
...
export function Jar({ tone = "matcha", size = 1, label = "小膠傲", labelEn = "JoyJar", floating = false, style }: any) {
  const st = JAR_STYLES.includes(style) ? style : "classic";
