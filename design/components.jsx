// components.jsx — shared JoyJar UI pieces. Exported to window.

// Cinnabar seal stamp 印章 — auspicious brand mark
function Seal({ size = 56, style = {} }) {
  return (
...
function LogoMark({ size = 46, className = "", style = {} }) {
  return (
...
function Label({ children, zh, className = "" }) {
  return (
...
function Btn({ children, variant = "primary", className = "", onClick, type = "button", disabled, ...rest }) {
  return (
    <button
      type={type}
      data-fish="1"
...
function Toggle({ on, onChange, labels = ["On", "Off"] }) {
  return (
...
function AdminField({ label, zh, value, onChange, placeholder, type = "text", wide, textarea }) {
  return (
...
Object.assign(window, { Seal, LogoMark, Wave, Label, Btn, QtyStepper, Toggle, AdminField });
